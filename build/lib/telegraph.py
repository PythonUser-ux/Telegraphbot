...
class BOT:
    def __init__(self, name, token, allowed=[], directory=None, commands=[]):
        self.name=name
        self.token=token
    ...
    def chat_step(self,order,message,question,answers,actions,unrecognized,next_steps=[]):
        ...
        if words>order:
            for num, answer in enumerate(answers):
                    ...
                    if input==answer or re.search(answer, input):
                        if self._deep_degree==order:
        ...
        """
        order: the degree of depth of the chat_step
        message: the current user message
        question: the message showed to the user at the beginning of the chat_step
        question è ciò che viene detto all'utente all'inizio del corrispondente chat_step
        answers: the list of proper answers              Hint: it is possible to use Regex
        actions: it is the list of the functions that correspond to the proper answers 
        unrecognized: it is the default reply to non proper answers
        next_steps: the list of all the sons of the chat_step
        """

    def lazy(self, message):
        pass

    def reset(self):                   # go back to the root node
        self._start=False
    ...
    def polling(self):
        for command in self.commands:
            @self._bot.message_handler(commands=[command.name])
        ...
        def edited_detector(message):
            if (message.chat.id in self.allowed) and self._start:
                self._bot.send_message(message.chat.id, "You modified a message")
        ...
        def main_sender(message):
            ...
            if (message.chat.id in self.allowed) and self._start:
                ...
                self._deep_degree+=1
        # self._bot.polling(timeout=10, long_polling_timeout=10)  # interval=0, timeout=20, long_polling_timeout=20
